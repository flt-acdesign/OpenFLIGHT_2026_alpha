...
function transform_aerodynamic_forces_from_wind_to_body_frame(D, Y, L, alpha_RAD, beta_RAD)
    # Force vector in wind frame. Reverted to the backup's empirically-
    # working sign and rotation composition (see
    # `2026_03_014_▶OpenFLIGHT_codex_backup1_new_aeromodel_ok`). The α/β
    # passed in use the simulator's effective sign convention, not the
    # textbook standard, so the rotation composition must match.
