...
function atmosphere_isa(height)
    ...
    # Sanitise the input so transient numerical excursions (NaN, overshoots past the
    # ISA76 ceiling, negative depths during collision recovery) cannot crash the
    # simulation loop.  Out-of-range values are clamped rather than thrown.
    if !isfinite(height)
        height = 0.0
    end
    height = clamp(height, -500.0, 84499.0)
    ...
    elseif 71000 <= height <= 84500  # Mesosphere 2
        ...
        # Should be unreachable because of the clamp above, but keep a safe
        # fallback instead of a hard throw so a single bad frame can't kill
        # the simulation loop.
        T = 214.65
        p = 3.95642
    end
